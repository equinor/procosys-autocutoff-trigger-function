...
description: Migrating Azure Functions from in-process to isolated worker model with .NET 8+ patterns
---
...
- **CRITICAL**: In isolated worker model, dependency tracking is NOT automatic
- Must explicitly configure Application Insights in Program.cs:
  ```csharp
  builder.Services
      .AddApplicationInsightsTelemetryWorkerService()
      .ConfigureFunctionsApplicationInsights();
  ```
- `AddApplicationInsightsTelemetryWorkerService()` enables dependency tracking for Azure SDK calls (Storage, Service Bus, etc.)
- `ConfigureFunctionsApplicationInsights()` integrates Application Insights with the Functions host
- **Without this configuration**: Storage account dependencies (used by Durable Functions), HTTP calls, and other dependencies will NOT appear in Application Insights telemetry
- Requires `using Microsoft.Azure.Functions.Worker;` namespace
...
// REQUIRED: Enable Application Insights with dependency tracking
// Without this, storage account and other Azure SDK dependencies won't appear in telemetry
builder.Services
    .AddApplicationInsightsTelemetryWorkerService()
    .ConfigureFunctionsApplicationInsights();
...
builder.Services.AddSingleton(sp => 
    new ServiceBusClient(builder.Configuration["AzureWebJobsServiceBus"]));
